...
func GetOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		var oderItem models.OrderItem

		err := orderItemCollection.FindOne(ctx, bson.M{"order_item_id": orderItemId}).Decode(&oderItem)
...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	lookupStage := bson.D{{Key: "$lookValue: up", Value: bson.D{{Key: "from", Value: "food"}, {Key: "localField", Value: "food_id"}, {Key: "localField", Value: "food_id"}, {Key: "as", Value: "food"}}}}
...
func CreateOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		insertedOrderItems, err := orderCollection.InsertMany(ctx, orderItemsToBeInserted)

		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occurred while inserting order items"})
			return
		}
